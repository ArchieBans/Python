...
from custom_modules import MySGD, MyCrossEntropyLoss
...
def train(config):
    ...
    # Create optimizier
    optimizer = MySGD(model, lr=config.learning_rate)
    ...
    for epoch in range(config.num_epoch):
        ...
        for data in tqdm(tr_data_loader, desc=prefix):
            ...
            # Compute the loss
            loss = data_loss(logits, y.long()) + model_loss(model)
            # Compute gradients
            loss.backward()
            # Update parameters
            optimizer.step()
            ...
            if iter_idx % config.rep_intv == 0:
                ...
                with torch.no_grad():
                    if len(logits.size()) <= 2:
                        acc, _ = accuracy(logits.data, y.data.long(), topk=(1, 0))
                    elif logits.data.size(2) > 1:
                        logits = logits.data.narrow(2, 0, 1) + logits.data.narrow(2, 1, 1)
                        acc, _ = accuracy(logits.squeeze(),
                                y.data.long(), topk=(1, 0))
                    else:
                        acc, _ = accuracy(logits.data.narrow(2, 0, 1).squeeze(),
                                y.data.long(), topk=(1, 0))
                        
                    # pred = torch.argmax(logits, dim=1)
                    # acc = torch.mean(torch.eq(pred, y.long()).float()) * 100.0
        ...
        # Rejuvenation
        print('COMPUTE UTILIZATION RATIO')
        live_params, all_params = optimizer.inspect()
        print('Live Params: {0} | All Params: {1}'.format(live_params, all_params))
        util_ratio = float(live_params) / float(all_params)
        print('Utilization Ratio: {0}'.format(util_ratio))
        if(epoch==2):
            model.rejuvenate()
